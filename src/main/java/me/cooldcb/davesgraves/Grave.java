...
public class Grave {
    ...
    private void startGraveTimer() {
        ...
        if (secondsLeft < 0) {
            DavesGraves.dataManager.breakGrave(playerUUID, id);
            return;
        }
        Bukkit.getScheduler().runTaskLater(DavesGraves.getInstance(), () -> DavesGraves.dataManager.breakGrave(playerUUID, id), secondsLeft * 20L);
    }
}
